...
After implementing a feature, fixing a bug, or making any structural change
(new files, renamed components, new Makefile targets, new fields, new endpoints),
review and update **both** of the following before finishing:
...
Even small changes can affect docs — for example adding a field to the Go model
means the API example JSON in the README must be updated too.
...
2. Add a `TemplateInfo` entry to `handler.Templates` in `handler/resume.go`.
3. Add a matching entry to `TEMPLATES` in `apps/web/components/TemplateSelector.tsx`.
4. Use `{{ heading $ "key" "Default" }}` for section titles so headings are customisable.
5. Any custom template functions go in `renderer/renderer.go:newTemplateFuncs`.
6. Run `make api-thumbs` to generate the thumbnail PNG for the new template.
...
| `make thumbs` | Regenerate template thumbnail PNGs |
